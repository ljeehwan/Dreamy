...
export default {
  ...
  created: function () {
    // 뷰엑스로 받은 이름을 타겟에 넣어서
    const targetName = this.$store.state.userStore.targetName;
    //1. 스토어에 있는 GET_MEMBER dispatch한다. (네브 바의 마이 페이지 버튼은 어차피 
    // 자기 자신이기 때문에 이름을 내이름을 내려보내줌)
    // 응답을 변수에 담아서 저장
    this.$store.dispatch('userStore/GET_MEMBER', targetName)
    //vuex에서 정보 가져오기
    ...
    if (targetName === this.$store.getters["userStore/getUsername"]) {
